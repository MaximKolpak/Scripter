...
{
    public class X32Bus : ConsoleInput
    {
        public X32BusStrip Strip;

        public X32Bus()
        {
            Strip = new X32BusStrip();
            Strip.Parent = this;
            CreateMuteButton();
            CreateFader();
            CreateStereoLink();
        ...
        private void CreateEq()
        {
            ...
            for (int i = 0; i < X32.X32EqBandsBus; i++)
            {
        ...
        protected virtual void CreateMatrix()
        {
            ...
            for (int i = 0; i < X32.X32MaxMatrix - 1; i++)
            {
                Strip.Matrix[i].Address = "/mix/" + (i + 1).ToString("D2");
                Strip.Matrix[i].Parent = this;
                Strip.Matrix[i].Id = Id;

                Strip.Matrix[i].Fader = new X32Fader();
                Strip.Matrix[i].Fader.Address = "/level";
                Strip.Matrix[i].Fader.Parent = Strip.Matrix[i];
                Strip.Matrix[i].Fader.Id = Id;

                //Strip.Matrix[i].Pan = new X32PanDial();
                //Strip.Matrix[i].Pan.Address = "/pan";
                //Strip.Matrix[i].Pan.Parent = Strip.Matrix[i];
                //Strip.Matrix[i].Pan.Id = Id;

                Strip.Matrix[i].Mute = new X32MuteButton();
                Strip.Matrix[i].Mute.Address = "/on";
                Strip.Matrix[i].Mute.Parent = Strip.Matrix[i];
                Strip.Matrix[i].Mute.Id = Id;

            //    Strip.Matrix[i].TapPoint = new X32TapPoint();
            //    Strip.Matrix[i].TapPoint.Address = "/type";
            //    Strip.Matrix[i].TapPoint.Parent = Strip.Matrix[i];
            //    Strip.Matrix[i].TapPoint.Id = Id;
            //
            }
        }
